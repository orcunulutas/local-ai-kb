...
def test_service_endpoint_takes_precedence():
    ...
    with (
        patch("aikb.sources.exchange.client.Configuration") as mock_config_cls,
        ...
        assert (
            ...
            == "https://endpoint.invalid/EWS/Exchange.asmx"
        )
...
def test_missing_both_endpoint_and_server():
    ...
    with pytest.raises(
        ExchangeClientError, match="Either service_endpoint or server must be provided."
    ):
        client.connect()


@pytest.mark.parametrize(
    "auth_type_str, expected_auth",
    [
        ("NTLM", "NTLM"),
        ("ntlm", "NTLM"),
        ("Basic", "basic"),
